...
## Data modified from previous versions by SE Graham, June 2016
...
setwd("/Users/colleennell/Documents/R/suz/")

##Section 2: read in data

## Section 2.1: Pre

tag37616 <-read.csv("/Users/colleennell/Documents/R/suz/pre/37616.csv")
tag37623 <-read.csv("/Users/colleennell/Documents/R/suz/pre/37623.csv")
tag44366 <-read.csv("/Users/colleennell/Documents/R/suz/pre/44366.csv")
tag52674 <-read.csv("/Users/colleennell/Documents/R/suz/pre/52674.csv")
tag52675 <-read.csv("/Users/colleennell/Documents/R/suz/pre/52675.csv")
tag78500 <-read.csv("/Users/colleennell/Documents/R/suz/pre/78500.csv")
tag79786 <-read.csv("/Users/colleennell/Documents/R/suz/pre/79786.csv")
...
tag12607106 <-read.csv("/Users/colleennell/Documents/R/suz/post/12607106.csv")
tag12607107 <-read.csv("/Users/colleennell/Documents/R/suz/post/12607107.csv")
tag126070 <-read.csv("/Users/colleennell/Documents/R/suz/post/126070.csv")
tag12606905 <-read.csv("/Users/colleennell/Documents/R/suz/post/12606905.csv")
tag12606907 <-read.csv("/Users/colleennell/Documents/R/suz/post/12606907.csv")
...
#tag44359 <-read.csv("allfilters/44359.csv")
...
Pre.all.coords <- data.frame(x=Pre.all$Lon1, y=Pre.all$Lat1)
View(Pre.all.coords)
View(Pre.all)
...
Post.all.coords <- data.frame(x=Post.all$Lon1, y=Post.all$Lat1)
...
library(rgdal)
tagprj <- readOGR("/Users/sgraham/R/gst_hr_analysis2016/Tag_065_wgs84", "Tag_065")
...
## Section 3.1.2
##Changed hlim = c(0.365,1.5), grid=300
Pre.kd <- kernelUD(Pre.all.utm, h="LSCV",  hlim = c(0.03, 1.5), grid=200) #set grid=100
plotLSCV(Pre.kd)
...
png(filename="plots/Pre.all.png")
...
writeOGR(Pre.ver.50, "shapefiles/Pre.ver.50.shp",layer="Pre.ver.50", driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR(Pre.ver.95, "shapefiles/Pre.ver.95.shp",layer="Pre.ver.95", driver="ESRI Shapefile", overwrite_layer=TRUE)
...
write.csv(Pre.outputs, "outputs/PreOutputs.csv", row.names=FALSE)



############Post
## Section 3.2.2
Post.kd <- kernelUD(Post.all.utm, h="LSCV",  hlim = c(0.03, 1.5), grid=200) #set grid=100
plotLSCV(Post.kd)
...
png(filename="plots/Post.all.png")
...
##Write shapefile
writeOGR(Post.ver.50, "shapefiles/Post.ver.50.shp",layer="Post.ver.50", driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR(Post.ver.95, "shapefiles/Post.ver.95.shp",layer="Post.ver.95", driver="ESRI Shapefile", overwrite_layer=TRUE)


Post.outputs <- data.frame(Period="Post", Source="All", Method="LSCV", Bandwidth=Post.bw, A50=Post.Area.50, A95=Post.Area.95)
write.csv(Post.outputs, "outputs/PostOutputs.csv", row.names=FALSE)
...
## get coordinates as a dataframe and make a spatial object
Pre.unique <- unique(Pre.all$ArgosID)
Pre.ind.outputs <- c() #to hold results

for(i in 1:length(Pre.unique)){
  Pre.tmp <- subset(Pre.all, ArgosID==Pre.unique[i])
  print(i)
  ...
  ## Section 4.1.2
  Pre.tmp.kd <- kernelUD(Pre.tmp.utm, h="LSCV",  hlim = c(0.03, 1.5), grid=300) #set grid=300
  #plotLSCV(Pre.kd)
  ...
  filename50 <- paste("shapefiles/", "tag",Pre.unique[i],"percent50th", ".shp", sep="")
  filename95 <- paste("shapefiles/", "tag",Pre.unique[i],"percent95th", ".shp", sep="")
  ##Write shapefile
...
write.csv(Pre.ind.outputs , "outputs/Pre.ind.outputs .csv", row.names=FALSE)
...
for(i in 1:length(Post.unique)){
  ...
  ## Section 4.1.2
  Post.tmp.kd <- kernelUD(Post.tmp.utm, h="LSCV",  hlim = c(0.365, 1.5), grid=1000) #set grid=300
  #plotLSCV(Post.kd)
  ...
  filename50 <- paste("shapefiles/", "tag",Post.unique[i],"percent50th", ".shp", sep="")
  filename95 <- paste("shapefiles/", "tag",Post.unique[i],"percent95th", ".shp", sep="")
  ##Write shapefile
...
write.csv(summary.all , "outputs/summaryall.csv", row.names=FALSE)



save.image("homerange5.RData")
